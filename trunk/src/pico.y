...
comando: lvalue '=' expr {	Node* filho2 = create_node( @2.first_line, attr_node, $2, NULL, NULL);
				...
				{		     
				     ...
				     struct tac* new_instruction = create_inst_tac(variable->name, $3->local, "", "", NULL, "");
				...
				else
				{
					...
					struct tac *instruction1 = create_inst_tac(indirect_access, $3->local, "", "", NULL, "");
...
lvalue: IDF { 	$$ = create_node(@1.first_line, idf_node, $1, NULL, NULL);
	...
	| listaexpr ']'	{	Node* filho2 = create_node(@2.first_line, rightbracket3_node, $2, NULL, NULL);
				...
				struct tac *instruction1 = create_inst_tac(temp1, $1->local, "MUL", weight, NULL, "");				
...
listaexpr: listaexpr ',' expr   {	Node* filho2 = create_node(@2.first_line, comma_node, $2, NULL, NULL);
					...
					struct tac *instruction1 = create_inst_tac(temp, $1->local, "MUL", limit, NULL, "");
					append_inst_tac(&($1->code), instruction1);					
					...
					struct tac *instruction2 = create_inst_tac(temp2, temp, "ADD", $3->local, NULL, "");
					append_inst_tac(&($1->code), instruction2);
...
expr: expr '+' expr  {  Node* filho2 = create_node( @2.first_line, plus_node, $2, NULL, NULL);
			...
			struct tac* new_instruction = create_inst_tac($$->local, $1->local, "ADD", $3->local, NULL, "");
			append_inst_tac(&($3->code), new_instruction); 
    ...
    | expr '-' expr  {  Node* filho2 = create_node( @2.first_line, minus_node, $2, NULL, NULL);
			...
			struct tac* new_instruction = create_inst_tac($$->local, $1->local, "SUB", $3->local, NULL, "");
			append_inst_tac(&($3->code), new_instruction); 
    ...
    | expr '*' expr  {  Node* filho2 = create_node( @2.first_line, mul_node, $2, NULL, NULL);
			...
			struct tac* new_instruction = create_inst_tac($$->local, $1->local, "MUL", $3->local, NULL, "");
			append_inst_tac(&($3->code), new_instruction); 
    ...
    | expr '/' expr  {  Node* filho2 = create_node( @2.first_line, div_node, $2, NULL, NULL);
			...
			struct tac* new_instruction = create_inst_tac($$->local, $1->local, "DIV", $3->local, NULL, "");
			append_inst_tac(&($3->code), new_instruction); 
...
enunciado: expr { $$ = $1 ;}
	 ...
	 | WHILE '(' expbool ')' '{' acoes '}'  {  	
	 						Node* filho1 = create_node( @1.first_line, while_node, $1, NULL, NULL);
						    	...
						    	*return_line = (int **)malloc(sizeof(int *));
    	 ...
    	 | PRINTF '(' expr ')' {   	
					...
					struct tac* new_instruction = create_inst_tac("PRINT", $3->local, "", "", NULL, "");
					append_inst_tac(&($3->code), new_instruction);
					cat_tac(&($$->code), &($3->code));
			  	}
					...
					struct node_tac *instructions = $2->code;
				    	while (instructions->next != NULL)
				    		instructions = instructions->next;
    			    	      	...
    			    	      	*skip_line = (int **)malloc(sizeof(int *));
